class Fretboard {
    ...
    getThemeColors() {
        const theme = this.settings?.colorTheme || 'default_ii';
        const defaultColors = {
            background: '#FFFFFF',
            strings: '#888888',
            frets: '#444444',
        ...
        if (theme === 'custom' && this.settings?.customColors) {
            ...
            resultColors = { ...defaultColors, ...palette };
        }

        if (this.settings?.forceWhiteBackground) {
            resultColors.background = '#FFFFFF';
        }
